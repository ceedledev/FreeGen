<?php 
    ...
    if(empty($code)){
        ...
        $dedipass = file_get_contents('http://api.dedipass.com/v1/pay/?public_key=dfcba2656f3ccc720b0582169c98de13&private_key=8625d56d3dd746bce360a7089016b1c7a88c&code=' . $code); 
        ...
        if($dedipass->status == 'success') {
            ...
            $resultat['content'] = "Le code ".$code." à été utilisé pour un montant d'une valeur de ".$dedipass->payout." EUR pour le grade GIANT";

            $now = date("Y-m-d H:i:s");
            $add = strtotime('+182 days');
            $expir = date("Y-m-d H:i:s", $add);

            
            $modif = $bdd->prepare('UPDATE membres SET grade = ?, expiration = ? WHERE id = ?');
            $modif->execute(array(3, $expir, $utilisateur['id']));

            $insert = $bdd->prepare('INSERT INTO paiements (id_user, grade, montant, code, date) VALUES (?, ?, ?, ?, ?)');
            $insert->execute(array($utilisateur['id'], "GIANT", $dedipass->payout, $code, $now));
